...
export default function AchievementsPanel() {
  ...
  return (
    <FadeIn>
      <Card id="conquistas" padding="lg" hover>
        <CardHeader
          subtitle="Achievement Engine"
          title="Conquistas"
          action={
            <Badge tone="blue">
              <CountUp value={view.unlockedCount} />/<CountUp value={view.total} />
            </Badge>
          }
        />
        ...
        {summary?.newlyUnlocked?.length ? (
          <Pulse active color="#22c55e" className="mb-3 block w-full">
            <p className="rounded-lg bg-emerald-50 px-3 py-2 text-xs text-emerald-700 ring-1 ring-emerald-100">
        ...
        {display.length === 0 ? (
          ...
          <SlideUp stagger as="ul" className="space-y-2">
            {display.map((a) => {
              const unlocked = a.status === 'unlocked'
              return (
                <SlideUpItem key={a.id} as="li">
                  <AchievementGlow active={unlocked} intensity="md">
                    <div
                      className={[
